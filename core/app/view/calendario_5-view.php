...
function generarTabla($contacts)
{
  ...
  if (count($contacts) > 0) :
    $tabla .= '<div class="table-responsive col-12">
                    <table class="table table-striped table-bordered table-hover datatableC col-12">
                        <thead>
                            <th>Linea</th>
                            <th>Hora</th>
                            <th>Descripcion</th>
    ...
    foreach ($contacts as $con) :
      ...
      $tabla .= '</td>
                        ...
                        <button type="button" class="btn btn-warning btn-sm open-modal" data-toggle="modal" data-target="#editModal' . $con->id . '" data-tab="linea' . $con->linea . '">
      ...
      $tabla .= '<div class="modal fade" id="editModal' . $con->id . '" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-body">
                  ...
                  <form method="post" id="addproduct_' . $con->id . '" action="./?action=idoall&opt=updateLyF&tab=linea' . $con->linea . '" role="form">
...
function generarContenidoPestanas($usuario)
{
  ...
  foreach ($pestanasMostrar as $index => $linea) {
    ...
    $contenidoPestanas .= '<div class="tab-pane fade col-12' . ($index == 0 ? ' show active' : '') . '" id="' . $tabId . '" role="tabpanel" aria-labelledby="' . $tabId . '-tab">';
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h1 class=""><b>Gestión de Averías</b></h1>
            <?php
            /* echo $usuario;
          ...
          <div class="card-body">
            <div class="container mt-2 col-12">
              <ul class="nav nav-tabs col-12" id="custom-tabs-three-tab" role="tablist">
                <?php
                $pestanasMostrar = [];
                if ($usuario == 1) {
                  $pestanasMostrar = [1, 3, 4, 12];
              ...
              <div class="tab-content mt-4 col-12" id="custom-tabs-three-tabContent">
...
<script>
  $(document).ready(function() {
    // Almacenar la última pestaña activa visitada en el almacenamiento local
    ...
    // Restaurar la pestaña activa después de la redirección
    var tabFromUrl = getParameterByName('tab');
    if (tabFromUrl) {
      $('.nav-link').removeClass('active');
      $('.tab-pane').removeClass('show active');
      $('#' + tabFromUrl + '-tab').addClass('active');
      $('#' + tabFromUrl).addClass('show active');
    } else {
      // Restaurar la última pestaña activa visitada al cargar la página
      var lastActiveTab = localStorage.getItem('lastActiveTab');
      if (lastActiveTab) {
        $('.nav-link').removeClass('active');
        $('.tab-pane').removeClass('show active');
        $('#' + lastActiveTab).addClass('active');
        $('#' + lastActiveTab + '-tab').addClass('active');
      }
    }
  });
</script>
