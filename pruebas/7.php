...
if ($result->num_rows > 0) { // 9
    ...
    while ($row = $result->fetch_assoc()) { // 10
        ...
        $posicionPierde = stripos($texto, "Pierde"); // 15

        // Si se encuentra la frase "vueltas perdidas" en el texto
        if ($posicionPierde !== false) {
            // Incrementar el contador de vueltas perdidas
            $totalVueltasPerdidas++;
