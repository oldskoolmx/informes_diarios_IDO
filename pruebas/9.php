 ...
 */
        ...
        /*  if (preg_match("/\b(0?\.\d+|\d+(\.\d+)?)\s*vuelta\b/i", $subcadena, $matches)) {
        ...
        //if (preg_match("/\b(\d*\.?\d+)\s*vueltas?\b/i", $subcadena, $matches)) {
        if (preg_match("/[\d\.]+ vueltas?/", $subcadena, $matches)) {
            //if (preg_match("/\b(\d*\.?\d+)\s*vuelta\b/i", $subcadena, $matches)) {
            // Extraer solo el número de vueltas encontrado
            //$vueltasPerdidas = floatval(preg_replace('/[^0-9.]/', '', $matches[0]));
        ...
        /*  if (preg_match("/(?:\d+\.?\d*|\.\d+) vuelta/i", $subcadena, $matches)) {
            // Extraer solo el número de vueltas encontrado
            $vueltasPerdidas = floatval(preg_replace('/[^0-9.]/', '', $matches[0]));
            // Sumar las vueltas perdidas al total
            $totalVueltasPerdidas += $vueltasPerdidas;
